...
# for IP camera, use the following line instead:
cap = cv2.VideoCapture("http://192.168.56.247:8080/video")
...
def get_latest_frame(cap):
    for _ in range(5):  # discard a few frames
        cap.read()
    return cap.read()
...
def update():
    ...
    img = cv2.resize(img, (480, 360))  # Smaller = faster
